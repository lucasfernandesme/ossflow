...
import { Student, BeltInfo } from '../types';
...
const BeltGraphic: React.FC<{ belt: string; stripes: number; belts: BeltInfo[]; className?: string }> = ({ belt, stripes, belts, className = "" }) => {
  const beltInfo = belts.find(b => b.name === belt || b.name.includes(belt)) || belts[0] || { color: '#FFFFFF', name: '' };
      ...
      >
        ...
        <div className="flex-1 relative">
          {beltInfo.secondaryColor && (
            <div className="absolute inset-x-0 top-1/3 h-1/3" style={{ backgroundColor: beltInfo.secondaryColor }}></div>
          )}
        </div>
...
const StudentList: React.FC<StudentListProps> = ({
  ...
  const getEligibility = (student: Student) => {
    ...
    // Se graduou hoje, não mostrar na lista
    if (student.lastGraduationDate) {
      const graduationStr = student.lastGraduationDate.split('T')[0];
      if (graduationStr === today) return null;
    }

    if (belts.length === 0) return null;

    const beltCriteria = belts.find(b => b.name === student.belt || b.name.includes(student.belt)) || belts[0];
    if (!beltCriteria) return null;

    const stripeReq = beltCriteria.freqReq > 0 ? beltCriteria.freqReq : 1;
    const classesReqTotal = beltCriteria.classesReq;

    if (student.stripes < 4) {
      // 0-3 Stripes
      if (student.totalClassesAttended >= stripeReq) {
      ...
      if (student.totalClassesAttended >= finalLegGoal) {
  ...
  const filteredStudents = useMemo(() => {
    ...
    return result;
  }, [students, searchTerm, activeFilter, belts]);
        ...
        {filteredStudents.map((student) => {
          ...
          return (
            ...
            >
              ...
              {/* DEBUGGING MOBILE */}
              {(student.name.toLowerCase().includes('karini') || student.name.toLowerCase().includes('joao')) && (
                <div className="text-[8px] text-red-500 text-left px-4 pb-2">
                  St: {student.stripes} |
                  Belt: {student.belt} |
                  FreqReq: {belts.find(b => b.name === student.belt || b.name.includes(student.belt))?.freqReq} |
                  ClassesReq: {belts.find(b => b.name === student.belt || b.name.includes(student.belt))?.classesReq} |
                  Total: {student.totalClassesAttended} |
                  FinalGoal: {student.stripes >= 4 ? Math.max((belts.find(b => b.name === student.belt || b.name.includes(student.belt))?.freqReq || 0), (belts.find(b => b.name === student.belt || b.name.includes(student.belt))?.classesReq || 0) - 4 * (belts.find(b => b.name === student.belt || b.name.includes(student.belt))?.freqReq || 0)) : 'N/A'} |
              ...
              return (
                <tr key={student.id} className="hover:bg-zinc-50/50 transition-colors group cursor-pointer" onClick={() => onEditStudent(student)}>
                  ...
                  <td className="px-6 py-4 text-center">
                    ...
                    {/* DEBUGGING */}
                    {(student.name.includes('Karini') || student.name.includes('Joao')) && (
                      <div className="text-[8px] text-red-500 text-left mt-1">
                        St: {student.stripes} |
                        Belt: {student.belt} |
                        Dt: {student.lastGraduationDate || 'None'} |
                        Req: {belts.find(b => b.name === student.belt || b.name.includes(student.belt))?.freqReq}
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-4 text-center">
