          ...
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
              ...
              <div className="bg-zinc-50 p-6 rounded-2xl border border-zinc-100 flex flex-col items-center gap-4">
                ...
                <p className="text-zinc-950 dark:text-white font-black uppercase tracking-tight text-lg">{formData.name || 'Nome da Faixa'}</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 focus:outline-none focus:ring-2 focus:ring-zinc-950/10 dark:focus:ring-white/10 focus:border-zinc-950 dark:focus:border-white transition-all text-sm font-medium bg-white dark:bg-zinc-800 text-zinc-950 dark:text-white"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold uppercase text-zinc-500 dark:text-zinc-400">Ordem (Posição)</label>
                  <input
                    type="number"
                    className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 focus:outline-none focus:ring-2 focus:ring-zinc-950/10 dark:focus:ring-white/10 focus:border-zinc-950 dark:focus:border-white transition-all text-sm font-medium bg-white dark:bg-zinc-800 text-zinc-950 dark:text-white"
                ...
                <div className="space-y-1.5 md:col-span-2">
                  <div className="p-4 rounded-xl bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100/50 dark:border-blue-900/20 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-[10px] font-bold uppercase text-blue-500 dark:text-blue-400 mb-1.5 block">Aulas para Trocar de Faixa (Total)</label>
                      <div className="relative">
                        <input
                          type="number"
                          placeholder="0"
                          className="w-full pl-4 pr-10 py-3 rounded-xl border border-blue-200 dark:border-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 focus:border-blue-500 dark:focus:border-blue-400 transition-all text-sm font-bold text-blue-900 dark:text-blue-200 bg-white dark:bg-zinc-800"
                          value={formData.classesReq}
                          onChange={(e) => setFormData({ ...formData, classesReq: parseInt(e.target.value) || 0 })}
                        />
                        <div className="absolute right-3 top-1/2 -translate-y-1/2 text-blue-300 dark:text-blue-600 pointer-events-none">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2v20M2 12h20" /></svg>
                        </div>
                      </div>
                      <p className="text-[10px] text-blue-400 dark:text-blue-500 mt-1 leading-tight">Total acumulado necessário para se graduar nesta faixa.</p>
                    </div>
                    <div>
                      <label className="text-[10px] font-bold uppercase text-blue-500 dark:text-blue-400 mb-1.5 block">Aulas para Próximo Grau</label>
                      <div className="relative">
                        <input
                          type="number"
                          placeholder="0"
                      ...
                      <p className="text-[10px] text-blue-400 dark:text-blue-500 mt-1 leading-tight">Frequência necessária para cada grau (stripe).</p>
                    </div>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold uppercase text-zinc-500 dark:text-zinc-400">Cor Principal</label>
                  <div className="flex items-center gap-3">
                    <input
                      type="color"
                      className="w-12 h-12 rounded-xl cursor-pointer border-0 p-0 shadow-sm ring-1 ring-zinc-200 dark:ring-zinc-700"
                    ...
                    <input
                      ...
                      value={formData.color}
                      onChange={(e) => setFormData({ ...formData, color: e.target.value })}
                      className="flex-1 px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 text-sm font-mono uppercase focus:outline-none focus:border-zinc-950 dark:focus:border-white transition-all bg-white dark:bg-zinc-800 text-zinc-950 dark:text-white"
                    />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold uppercase text-zinc-500 dark:text-zinc-400">Cor Secundária (Opcional)</label>
                  <div className="flex items-center gap-3">
                    <input
                      type="color"
                      className="w-12 h-12 rounded-xl cursor-pointer border-0 p-0 shadow-sm ring-1 ring-zinc-200 dark:ring-zinc-700"
            ...
            {/* Modal Footer */}
            <div className="p-6 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex gap-3">
              <button
                onClick={handleClose}
                className="flex-1 py-3.5 rounded-xl font-bold text-zinc-600 dark:text-zinc-300 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-700 hover:text-zinc-900 dark:hover:text-white transition-all uppercase tracking-wide text-xs"
              >
                Cancelar
              </button>
              <button
                onClick={handleSave}
                className="flex-[2] bg-zinc-950 dark:bg-white text-white dark:text-zinc-950 py-3.5 rounded-xl font-black uppercase tracking-widest hover:bg-black dark:hover:bg-zinc-200 transition-all active:scale-95 text-xs shadow-xl shadow-zinc-950/10 dark:shadow-white/10 flex items-center justify-center gap-2"
              >
      ...
      <div className="grid grid-cols-1 gap-4 lg:gap-6">
        {belts.map((belt, index) => (
          <div
            ...
            className="group bg-white dark:bg-zinc-900 p-5 lg:p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:shadow-lg transition-all cursor-pointer hover:border-zinc-950/20 dark:hover:border-white/20 relative overflow-hidden"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-transparent to-zinc-50 dark:to-zinc-950 opacity-0 group-hover:opacity-100 transition-opacity"></div>
            <div className="flex-1 relative z-10">
              <div className="flex items-center gap-3">
                <h3 className="text-lg font-black text-zinc-950 dark:text-white uppercase tracking-tight">{belt.name}</h3>
                <span className="px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 text-[10px] font-bold text-zinc-500 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">#{belt.position}</span>
              </div>

              <div className="flex items-center gap-4 mt-3">
                <div className="flex items-center gap-1.5 text-zinc-700 dark:text-zinc-300">
                  <svg className="text-zinc-400 dark:text-zinc-500" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
                  <span className="text-xs font-bold"><span className="text-zinc-950 dark:text-white">{belt.classesReq}</span> <span className="text-zinc-400 dark:text-zinc-500 font-medium">aulas totais</span></span>
                </div>
                <div className="w-1 h-1 bg-zinc-300 dark:bg-zinc-600 rounded-full"></div>
                <div className="flex items-center gap-1.5 text-zinc-700 dark:text-zinc-300">
                  <svg className="text-zinc-400 dark:text-zinc-500" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>
                  <span className="text-xs font-bold"><span className="text-zinc-950 dark:text-white">{belt.freqReq}</span> <span className="text-zinc-400 dark:text-zinc-500 font-medium">p/ grau</span></span>
                </div>
              </div>
            </div>
